    ...
    *end_l=false;
    ...
    for(int i=1; i<dane.size(); i++)
    {
        for(int x=1; x<dane[i].size(); x++)
        {
            ...
            {
                string fake=rel(dane[i][x]);
                for(int sd=0; sd<(fake.length()/v_dataf->length()); sd++)
                {
                    if(*v_dataf==fake.substr(sd*v_dataf->length(),(sd*v_dataf->length())+v_dataf->length()))
                    {
                        cout<<i<<".|";
                        for(int m=1; m<dane[i].size(); m++)
                        {
                            cout<<rel(dane[i][m])<<" |";
                            *end_l=true;
                        }

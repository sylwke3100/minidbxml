...
int save_database(int u,int wid)
{
    ...
    x->open("./.data/base.xml",ios::in|ios::out|ios::trunc);
        ...
        *x<<"<header>"<<endl<<"<Program>MiniDBXML</Program>"<<endl<<"<Version>0.0.5 - Alpha </Version>"<<endl<<"<Time_Date>"<<asctime (timeinfo)<<"</Time_Date>"<<endl<<"</header>"<<endl;
        *x<<"<tag>"<<endl;
        for(int y=1;y<=vid;y++)
        {
...
delete x;
cout<<"save base.csv in $run_path/.data"<<endl;
return view_menu();
}
...
"1.View Database"<<endl<<"2.Find entry."<<endl<<"3.Edit Database"<<endl<<"4.Edit Column Database"<<endl<<"5.Export to CSV"<<endl<<"q.Exit"<<endl;
char wybor;
cin>>wybor;
switch(wybor)
{
